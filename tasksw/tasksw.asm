...
tss0_seg				equ	gdt_tss0 - gdt_start
dispatcher_seg	equ gdt_tss_dispatcher - gdt_start
task1_seg				equ gdt_tss_task1 - gdt_start
task2_seg				equ gdt_tss_task2 - gdt_start
ldt_seg_ring3		equ gdt_ldtd - gdt_start
...
tss_dispatcher:
		...
		dw	data_seg_ring3 | RPL3		; (not used) ds selector
		dw	ldt_seg_ring3			; (not used) task ldt selector
tss_dispatcher_end:
...
;
; Local Descriptor Table (unused in this program)
;
ldt_start:
...
start_pm_ring0:							; now entered into the 16 bit-protected mode ring 0 from the real mode
		...
		mov ax, tss0_seg				; only operable at prililege level 0
		ltr ax
