...
gdt_cgate_print:						; Call gate descriptor
		...
		db	1										; word count (one parameter required here)
...
print_end:
		mov bp, sp
		mov ax, [bp + 4]					; read the parameter 
		mov es:(17 * 2), ax				; print it
		retf 2										; this routine accepts one word as a paramter. one word is 2 bytes here. Thus the value passed to retf is 2.
...
start_pm_ring3:
		...
		mov ah, textcolor2
		push ax										; pass one paramter (1 word) to the gate
