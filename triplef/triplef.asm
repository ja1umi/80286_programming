...
idt:                        ; interrupt descriptor table. intentionally left invalid
    dw 0                    ; limit == 0
    dw 0
    db 0
    db 0
...
restart_ptr     equ 0x467
shutdown_status equ 0x0f
jmp_request     equ 5
cmos_addr       equ 0x70
cmos_data       equ 0x71

    mov [restart_ptr+2], ax
    lea ax, restart_rm
    mov [restart_ptr], ax
    mov al, shutdown_status
    out cmos_addr, al
    mov al, jmp_request 
    out cmos_data, al
...
textcolor2  equ 0x1f        ; blue on white, no blink
...
;jmp loop
;    mov dl, 0
;    div dl                 ; or simply 'int 0' (divide error exception; interrupt #0)
    int 6                   ; cause an invalid opcode exception (interrupt #6)

restart_rm:
    xor ax, ax              ; remember that CS == 0
    mov ds, ax
    mov ax, 0xb800
    mov es, ax
    mov di, 80 * 2          ; second row on the screen
    lea si, hello_msg2
    mov ah, textcolor2
print2:
    lodsb
...
hello_msg2:
    db  "Welcome back to the cruel Real World!",0
